...
import { Eye, EyeOff, Check, X, Mail, User as UserIcon, ArrowLeft, Gift } from 'lucide-react';
...
// - Sticky CTA on mobile (thumb-reachable)
// - Real-time password rules checklist (Binance pattern)
// - Segmented Email / Phone tab (Phone disabled placeholder for now)
// - Inline email preview & visual strength bar
// ============================================================================
export default function RegisterPage() {
  ...
  // --------------------------------------------------------------------------
  // Password rules (Binance-style live checklist)
  // --------------------------------------------------------------------------
  const rules = useMemo(() => ({
  ...
  const allValid = emailValid && nickValid && rules.length && rules.letter && rules.number && rules.match && agreeTerms;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!rules.length || !rules.letter || !rules.number) return setError(t('auth.passwordRulesFail'));
  ...
  return (
    <div className="min-h-screen bg-exchange-bg flex flex-col">
      {/* ---------- Top bar (Binance-style: back arrow + right-aligned login link) ---------- */}
      <header className="sticky top-0 z-30 bg-exchange-bg/95 backdrop-blur border-b border-exchange-border/40">
        <div className="max-w-md mx-auto px-4 py-3 flex items-center justify-between">
          <Link to="/home" className="p-1 -ml-1 hover:bg-exchange-hover rounded-md transition-colors" aria-label="back">
      ...
      <main className="flex-1 flex justify-center">
        <div className="w-full max-w-md px-5 pt-6 pb-28 sm:pb-10">
          {/* Headline */}
          <div className="mb-6">
            <h1 className="text-2xl sm:text-3xl font-bold text-exchange-text leading-tight">
          ...
          {/* Bonus banner - Binance uses a reward badge near headline */}
          ...
          {/* Segmented Email / Phone tab (Upbit & Binance use this pattern) */}
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            >
              <div className="relative">
                <Mail size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-exchange-text-third pointer-events-none" />
                <input
                  type="email"
                  value={email}
            ...
            >
              <div className="relative">
                <UserIcon size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-exchange-text-third pointer-events-none" />
                <input
                  type="text"
                  value={nickname}
            ...
            >
              ...
              {/* Rules checklist - Binance pattern */}
            ...
            <FieldWrap
              ...
              hint={confirmPw.length > 0 && !rules.match ? t('auth.passwordMismatch') : ''}
              focused={focused === 'pw2'}
            >
              <div className="relative">
                <input
            ...
            {/* Referral code - collapsible (Binance pattern) */}
            <div>
              <button
                type="button"
              ...
              >
                ...
                <span className="text-exchange-text-third">({t('auth.optional')})</span>
                <svg
                  width="10"
                  height="10"
                  viewBox="0 0 10 10"
            ...
            <div className="space-y-2.5 pt-1">
              <label className="flex items-start gap-2.5 cursor-pointer group">
                ...
                <span className="text-xs text-exchange-text-secondary leading-relaxed">
                  ...
                  <Link to="/terms" className="text-exchange-yellow hover:underline mx-0.5">
                    {t('auth.termsOfService')}
                  </Link>
                  {t('auth.and')}
                  <Link to="/privacy" className="text-exchange-yellow hover:underline mx-0.5">
              ...
              <label className="flex items-start gap-2.5 cursor-pointer group">
                ...
                />
                <span className="text-xs text-exchange-text-secondary leading-relaxed">
                  {t('auth.agreeMarketing')} <span className="text-exchange-text-third">({t('auth.optional')})</span>
                </span>
              </label>
            </div>
            ...
            {/* Divider + social (placeholder-ready) */}
          ...
          <p className="text-center text-sm text-exchange-text-secondary mt-8">
            {t('auth.hasAccount')}{' '}
            <Link to="/login" className="text-exchange-yellow hover:underline font-semibold">
              {t('nav.login')}
            </Link>
          </p>
        </div>
      ...
      {/* Sticky bottom CTA - mobile only (Binance mobile pattern) */}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <label className="text-[13px] font-medium text-exchange-text-secondary">{label}</label>
        {valid === true && (
...
function RuleRow({ ok, label }: { ok: boolean; label: string }) {
  return (
    <li className={`flex items-center gap-1.5 transition-colors ${ok ? 'text-exchange-buy' : 'text-exchange-text-third'}`}>
      {ok ? <Check size={12} strokeWidth={3} /> : <span className="w-[12px] h-[12px] rounded-full border border-current inline-block" />}
      <span>{label}</span>
    </li>
  );
}
...
function SocialBtn({ label, icon }: { label: string; icon: React.ReactNode }) {
  return (
    ...
    >
      {icon}
      <span className="text-xs text-exchange-text-secondary hidden sm:inline">{label}</span>
    </button>
  );
}

// Tiny inline brand logos (avoid external icon deps)
function GoogleIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" aria-hidden>
      <path fill="#4285F4" d="M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844a4.14 4.14 0 0 1-1.797 2.716v2.259h2.908c1.702-1.567 2.685-3.874 2.685-6.615z" />
      <path fill="#34A853" d="M9 18c2.43 0 4.467-.806 5.956-2.184l-2.908-2.259c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332A8.997 8.997 0 0 0 9 18z" />
      <path fill="#FBBC05" d="M3.964 10.706A5.41 5.41 0 0 1 3.682 9c0-.593.102-1.17.282-1.706V4.962H.957A9.002 9.002 0 0 0 0 9c0 1.452.348 2.827.957 4.038l3.007-2.332z" />
      <path fill="#EA4335" d="M9 3.579c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0A8.997 8.997 0 0 0 .957 4.962L3.964 7.294C4.672 5.167 6.656 3.58 9 3.58z" />
    </svg>
  );
}
function AppleIcon() {
...
function KakaoIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden>
      <path fill="#FEE500" d="M12 3C6.48 3 2 6.58 2 11c0 2.83 1.84 5.32 4.6 6.74l-.94 3.45c-.1.36.28.64.6.44l4.12-2.73c.54.06 1.08.1 1.62.1 5.52 0 10-3.58 10-8C22 6.58 17.52 3 12 3z" />
    </svg>
  );
}
