    ...
    <div className="min-h-screen bg-exchange-bg flex flex-col">
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-8">
        <div className="w-full max-w-md">
          ...
          <p className="text-sm text-exchange-text-secondary mb-6">{t('auth.forgotDesc')}</p>
            ...
            <div className="space-y-4">
              ...
              {devUrl && (
                <div className="bg-exchange-card border border-exchange-border rounded-lg p-3 text-xs break-all">
                  <p className="text-exchange-text-third mb-1">DEV link (email provider not configured):</p>
                  <a href={devUrl} className="text-exchange-yellow underline">{devUrl}</a>
                </div>
              )}
              <Link to="/login" className="block text-center text-sm text-exchange-yellow hover:underline">
                {t('auth.backToLogin')}
              </Link>
            </div>
          ) : (
            <form onSubmit={submit} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <Mail size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-exchange-text-third" />
                  <input
                    type="email"
                    value={email}
