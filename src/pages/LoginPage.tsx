...
// - Sticky bottom CTA on mobile for thumb reach
// - Segmented Email / Phone tab
// - Social sign-in placeholders (Google / Apple / Kakao)
// - Caps Lock warning like Binance desktop
...
export default function LoginPage() {
  ...
  const [email, setEmail] = useState(() => localStorage.getItem('quantaex_last_email') || '');
  ...
  const onPwKey = (e: React.KeyboardEvent<HTMLInputElement>) => {
    const caps = typeof (e as any).getModifierState === 'function' && (e as any).getModifierState('CapsLock');
    setCapsOn(!!caps);
  };
        ...
        <div className="w-full max-w-md px-5 pt-6 pb-28 sm:pb-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            <div>
              ...
              <div className="relative">
                <Lock size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-exchange-text-third pointer-events-none" />
                <input
          ...
          <p className="text-center text-sm text-exchange-text-secondary mt-8">
            {t('auth.noAccount')}{' '}
            <Link to="/register" className="text-exchange-yellow hover:underline font-semibold">
              {t('nav.register')}
            </Link>
          </p>

          {/* Trust row - Binance shows lock icons */}
