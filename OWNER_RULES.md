# 지상명령 (OWNER RULES) — 절대 규칙

> ⚠️ 이 문서는 사장님(오너)의 **지상명령**입니다. 여기 적힌 규칙은 임의로 변경 금지.
> 코드가 이 문서와 어긋나면 **문서가 맞고 코드가 틀린 것**으로 간주하고 코드를 고친다.

---

## 0. 🚫 한글(한국어) 화면 표시 절대 금지 — 최우선·무조건

- **사용자 화면(UI)에 표시되는 그 어떤 텍스트도 한국어(한글)로 표시하지 않는다.** 그 어떤 경우에도, 어디에도 예외 없음.
- 토스트/알림/버튼/라벨/에러 메시지 등 **화면에 렌더링되는 모든 문자열은 영어(또는 사용자가 선택한 비-한국어 언어)** 로만 노출.
- 서버 응답의 `message` 필드에 한국어를 넣고 프론트가 그걸 그대로 표시하는 것도 **금지**. 서버는 영어 `message` + `error` 코드만 반환하고, 프론트는 **error 코드 → i18n**으로 현지화한다.
- 코드 주석은 한국어 가능(화면에 안 나오므로). 단, **JSX/문자열 리터럴로 화면에 나가는 한국어는 금지.**
- 한국어 번들(`ko.ts`)은 운영자 전용(숨김 locale)이며 일반 사용자 화면 기본값이 아니다. 기본/폴백 언어는 영어(`en`).

---

## 1. 관리자 인정 스테이킹 (실입금 + 인정금액) — 최우선 규칙

관리자가 회원의 스테이킹을 **실입금 + 인정금액**으로 개설한 경우:

| 항목 | 기준 금액 | 설명 |
|---|---|---|
| **데일리수당** | **총금액 (실입금 + 인정금액)** | 총금액 기준으로 데일리 수당 지급 |
| **매칭보너스** | **총금액 (실입금 + 인정금액)** | 몸값(self_usd)·볼륨·매칭 전부 총금액 기준 |
| **원금 반환 (만기/해지)** | **실입금만** | 인정금액은 반환하지 않고 **소멸** |

### 예시
- 관리자가 실입금 $1,000 + 인정금액 $10,000 = **총 $11,000**으로 개설
- → 데일리수당: **$11,000** 기준으로 계산
- → 매칭보너스: 몸값 **$11,000**, 하부 볼륨·매칭 전부 $11,000 기준
- → 만기 원금 반환: **$1,000(실입금)만** 반환, $10,000(인정금액)은 소멸

### 코드 위치 (2026-08-31 기준, 규칙과 일치 확인됨)
- 총금액 = `principal_usd` = `realUsd + bonusUsd` — `src/server/routes/admin.ts` (grant 핸들러)
- 데일리수당: `accruedUsd()`가 `principal_usd`(총금액) 사용 — `src/server/routes/earn.ts`
- 매칭 롤업: `rollStakeUpBinary(db, user_id, principalUsd, ...)` 총금액 기준 — `src/server/routes/admin.ts`
- 원금 반환: `returnedQta = realPrincipalQta`(실입금만) — `src/server/routes/earn.ts` (redeem)
- 실입금 보관 컬럼: `staking_positions.real_principal_usd`, 인정금액: `bonus_principal_usd`

---

## 2. 데일리수당 규칙

- **기준**: 진입금액(총금액) × 데일리 요율(daily_rate) × 경과일수
- **시작 시점**: **익일부터** (진입 당일은 0일치, 지급 없음)
- **지급 단위**: 하루 **1일치씩**
- **날짜 기준**: **한국 표준시(KST, UTC+9) 자정**에 날짜가 바뀔 때마다 +1일치
  - 토요일 진입 → 당일 0 → 일요일 1일치 → 월요일 2일치(일+월)
  - 진입한 시각과 무관하게 **한국 날짜가 바뀌면** 하루치 발생
- **상한**: term_days(만기)까지만 적립
- **★ 포지션별 개별 계산 (2026-09-04 지상명령)**: 한 회원이 서로 다른 날 여러 번 진입하면(예: 오늘 $1,000, 한 달 뒤 $5,000), **각 포지션은 자기 금액·자기 요율(daily_rate)·자기 기간(term_days)·자기 시작일(created_at)로 완전히 개별 계산**한다. 절대 합산해서 뭉뚱그리지 않는다. (코드: `accruedQta()`/`accruedDays()`가 포지션 `p`별로 계산, positions API가 포지션마다 개별 map 처리.)
- **코드**: `accruedUsd()` + `kstDayIndex()` — `src/server/routes/earn.ts`
- **중복 지급 방지**: claim 시 `accrued_dividend_usd` 스냅샷 원자 가드 (claim-first)

---

## 3. 매칭보너스 규칙 (바이너리 좌/우)

- **기준**: 소실적(약한 레그 = min(좌 볼륨, 우 볼륨))
- **지급 방식**: 소실적이 각 **단계(티어) 목표에 도달할 때, 그 단계 1회만** 지급
- **단계별 목표·요율 (5단계)**:

| 소실적 도달 | 요율 | 단계 지급액 |
|---|---|---|
| $1,000 | 3% | $30 |
| $5,000 | 4% | $200 |
| $10,000 | 5% | $500 |
| $50,000 | 6% | $3,000 |
| $100,000 | 8% | $8,000 |

- **1회성**: 이미 지급한 단계는 재지급 없음 (`matched_usd` = 지급한 최고 목표, 원자 가드)
- **자격**: 본인이 스테이킹(몸값 self_usd > 0)해야 매칭 수령 가능
- **★ 볼륨은 무한대 (2026-09-04 정정)**: 좌우 볼륨(`left_usd`/`right_usd`)은 **상한 없이 하부 실적 전부 반영**. 이전의 "레그당 2× 몸값 볼륨 하드캡·드롭"은 **폐기**. 예) namim 몸값 $1,000, 왼쪽에 3명 총 $6,000 → `left_usd = $6,000`(전부 반영).
- **★ 매칭수당 총 한도 = 몸값 × 2 (2026-09-04 정정)**: 한 회원이 **평생 받을 수 있는 매칭수당 누계**는 **본인 몸값(self_usd)의 2배(USD)** 를 절대 초과 못 함(좌우 통합 상한). 소실적(min(좌,우)) 기준으로 티어 매칭을 계산하되, 누계가 `2 × self_usd`에 도달하면 그 이후는 지급 0. 예) 몸값 $1,000 → 매칭 평생 한도 $2,000. 좌 $500k/우 $1M라 소실적 $500k 티어가 $20,000을 주더라도 **$2,000까지만** 수령. 추후 본인이 몸값을 늘리면 한도가 커져 재개될 수 있음.
- **★ 몸값(self_usd)은 진입 시마다 누적 합산 (2026-09-04 지상명령)**: 본인 계정에서 **서로 다른 날 여러 번 진입**하면 각 진입 금액은 몸값에 **누적 합산**된다. 데일리 배당은 포지션별로 개별 계산(위 2번 규칙)이지만, **몸값(self_usd)은 모든 포지션 진입금액의 합**이다.
  - 예) 오늘 $1,000 진입(요율 A, 기간 A) → 몸값 $1,000. 한 달 뒤 별도로 $5,000 진입(요율 B, 기간 B) → **$5,000 진입한 그날부터 몸값 = $6,000**($1,000 + $5,000).
  - 이때 두 포지션의 **데일리는 각자의 요율·기간·시작일로 절대 개별 계산**(합산 금지). 하지만 **몸값 총액은 $6,000**으로 인정된다.
  - **하부 진입 상한 = 몸값 × 2**: 몸값 $6,000이 되는 순간부터 **직속 좌/우 하부는 각각 최대 $12,000까지 진입** 가능(매칭수당 총 한도 기준도 $12,000으로 상승).
  - (코드: `rollStakeUpBinary()`가 `!skipSelf`일 때 `self_usd = self_usd + ?`로 누적 — `src/server/lib/binary-matching.ts:317`. 매칭 한도·하부 기준 = 누적된 `selfUsd × 2`.)
- **★ 데일리와 별개 (2026-09-04)**: 위 매칭 200% 한도는 **데일리 배당과 완전 별개**. 데일리는 한도 없이 매일(스테이킹 기간 내) 지급.
- **한도 추적 기준**: 매칭은 QTA로 지급되나 시세 변동 때문에 한도는 **지급 시점 USD 가치(`bonus_usd`) 누계**로 관리 (`SUM(bonus_usd) < self_usd × 2`).
- **코드**: `MATCH_TIERS`, `runMatchForUser()`(매칭 2× 몸값 USD 누계 clamp), `rollStakeUpBinary()`(볼륨 무한 롤업) — `src/server/lib/binary-matching.ts` (웹서버 동기) + `cron-worker/src/binary-matching.ts` (안전망)

---

## 4. QTA 거래/시장운영 정책 (2026-09-05 지상명령)

QTA/USDT 마켓에만 적용. 회사(마켓메이커) = **mm-bot-a / mm-bot-b** (봇 계정, 데이터 수정 시 제외 대상).

- **★ 테더(USDT) 시세 = 1,450원 고정** (별도 지시 전까지). 매수·매도 환산 모두 1 USDT = 1,450 KRW. 코드 상수 `USDT_KRW_RATE = 1450`.
- **★ 24h 평균 하한선 (핵심 어뷰징 방어)**: **전일(어제 KST 0~24시) 거래량가중 평균 체결가** 이하 가격에서는 **어떤 매도도 체결 불가**(회원끼리든 회사든 전부). 전일 체결 이력이 없으면 **관리가격(`price_center`)** 을 하한선으로 사용.
  - 이유: 누군가 1원에 매도 → 회사가 (5만원어치) 사줌 → 그 사이 1원에 1억원어치 매수로 후려쳐 사가려는 자전거래 차단. 1원은 24h 평균 한참 아래이므로 **어떤 체결도 안 됨**.
  - 코드: `qtaPriceFloor()` + `matchOrder()` 체결 루프에서 `tradePrice < floor`이면 스킵 — `src/server/routes/order.ts`.
- **★ 매수 (회원이 QTA 삼) = 무제한**: 1 USDT=1,450원 환산, 나눠 사도 됨.
  - 체결 순서: **① 순수 매도인(일반 회원) 물량 먼저 → ② 부족분은 회사(mm-bot-a)가 매수인 주문가격 그대로 공급**해 100% 체결.
  - 단 **매수 주문가격이 현재 시세(mid) 미만이거나 24h 하한선 미만이면 회사는 공급 안 함** → 그 매수는 미체결로 방치("호가 시세 이하면 사주지 말라").
- **★ 매도 (회원이 QTA 팖)**: 시세 이하로 내놔도 **회사(mm-bot-b)가 무조건 실시간 매수**해 즉시 팔리게 해줌 — **단 회원 1인당 하루 5만원어치(= 50000/1450 ≈ 34.48 USDT)까지만**.
  - **★ 초과분 자동취소 (2026-09-05 정책수정)**: 하루 한도 5만원어치가 팔리면, 그 회원의 **남은 매도 주문은 시스템이 자동으로 전량 취소** — **매도호가창에서 제거 + 잠긴 QTA를 `available`로 환불**. (이전 '방치'에서 변경. 회원이 직접 취소할 필요 없음.)
  - 회사가 사줄 때 **매도인이 부른 가격(ask price)** 으로 체결(매도인은 자기 가격에 팔림), 단 24h 하한선 이상일 때만.
  - 개인당 한도 추적: 오늘 회사(mm-bot)가 그 회원에게서 사준 USDT 누계 기준. 코드 상수 `MM_MEMBER_BUY_BUDGET_USDT = 50000/1450`.
- **★ 체결 타이밍 = 즉시 아님, 1분 이내**: 회원 매수/매도 주문은 즉시 접수·호가 등재만 하고, 회사 개입 체결은 **1분 주기 cron(`* * * * *`)** 이 처리 → 주문 후 **1~59초 내 체결**. (Cloudflare 환경 안정성 위해 대기열 방식. 로직은 서버 `qta-mm-tick`에 있고 cron은 트리거만.)
- **코드**: `qtaPriceFloor()`/`matchOrder()`/`qta-mm-tick`(Step1 매도 실시간매수·개인5만원, Step1.5 매수 즉시공급) — `src/server/routes/order.ts`; cron 1분 트리거 — `cron-worker/wrangler.jsonc` + `cron-worker/src/index.ts`(`qtaMmTick`).

---

## 5. QTA 일일 시세 플랜 (Day Plan) — 2026-09-10 지상명령

> "오늘은 0.006을 중심으로 오르내리다가 마감을 0.0058 정도에 끝내라"

- **★ 하루 단위 지시를 자동 실행**: 관리자(또는 코드 기본값)가 **날짜(KST)·중심가·변동폭·마감가·마감 시작/도달 시각**을 넣으면 1분 mm-bot 틱이 다음 순서로 시세를 만든다.
  1. **ramp** — 활성화 시점의 마지막 체결가 → 중심가로 `ramp_minutes`(기본 90분) 동안 완만히 이동 (한 번에 점프 금지, 작은 캔들 연속).
  2. **oscillate** — 중심가 ± `band_pct`(기본 2.5%) 안에서 랜덤워크 (가장자리 15%에서 되돌림, 에지 고정 금지).
  3. **close** — `close_start`(기본 23:00 KST)부터 `close_end`(기본 23:55 KST)에 정확히 마감가에 도달하도록 선형 하강/상승, 노이즈는 점차 0.
  4. **hold** — close_end ~ 24:00 KST: 마감가에 고정.
  5. **carry** — 다음 날 새 플랜이 들어오기 전까지 **마감가 ± carry_band_pct(기본 1%)** 유지 (밤새 옛 관리가격으로 흘러내리지 않음).
- **저장 위치**: `system_state.key = 'qta_day_plan'` (JSON). 관리자 Clear 시 tombstone(`{cleared:true}`)이 남아 기본 플랜이 다시 심어지지 않음.
- **코드 기본값**(배포 즉시 적용, DB 입력 불필요): `DEFAULT_PLAN` = 2026-09-10 / 중심 0.006 / ±2.5% / 마감 0.0058 / 23:00→23:55 / 램프 90분 / 캐리 ±1% — `src/server/lib/qta-day-plan.ts`.
- **관리자 API/UI**: `GET/PUT/DELETE /api/admin/coins/QTA/day-plan`, 관리자 → Coins 탭 상단 "QTA Day Plan" 패널.
- **★ 다중 날짜 스케줄 (2026-09-14)**: 관리자 → Coins → "QTA Day Plan Schedule" → Edit week: 오늘부터 3~14일 표에 날짜별 O/C/H/L/Close·마감시각·램프 입력 후 Save. 저장된 날은 **그 날 00:00 KST에 자동 승격**(오늘 날짜는 즉시 적용). 우선순위: 스케줄 > 코드 기본 플랜 > 전날 carry. 저장 위치 `system_state.qta_day_plan_schedule`. 'Copy row 1 to all', 'Chain opens(시가=전날 종가)' 보조 버튼.
- **★ 관리자 OHLC 입력 (2026-09-14)**: 달러 기준 **Open(시가, 비우면 현재가) / Centre(중심) / High(최고가, 하드 상한) / Low(최저가, 하드 하한) / Close(종가)** + 마감 시작·도달 시각 + 램프 분. High/Low를 주면 등락폭(band)은 자동 산출되고, 램프가 범위에 진입한 뒤에는 어떤 틱(진동·덤프·마감·캐리)도 [Low, High] 밖으로 안 나감. 입력마다 원화(1,450) 환산 표시.
- **연관 규칙 조정**:
  - 플랜 활성 중 `coins.price_usd`(헤더 시세) 클램프는 관리형 밴드가 아니라 **플랜 엔벨로프**(중심가·마감가·시작가의 min/max ± band) 사용 — 옛 상한(0.004998)에 못 박히지 않게.
  - **24h 하한선(4번 규칙)은 회원이 관여한 체결에만 적용**. mm-bot-a ↔ mm-bot-b 코스메틱 캔들은 예외 (회원 어뷰징 방어 목적이므로 회사 봇끼리는 무관). 회원 매도/매수 체결에는 하한선 그대로 적용.
  - 플랜이 없거나 Clear 되면 이전 관리형(center ± band) 랜덤워크로 복귀.
- **참고**: 스테이킹 기준가(2번/`qtaStakeBasisPrice`)는 6원 고정 기간(~09-11) 이후 `price_center`를 계속 사용. 플랜과 별개이므로 필요 시 별도 지시.

---

## 6. ★★★ 영구명령 — QTA 회원 매도 하루 5만원 절대 상한 (2026-09-12) ★★★

> **"5만원 이상 절대 안 된다. 앞으로 영구명령이다."** — 이 규칙은 어떤 다른 규칙·기능·요청보다 우선한다. 어떤 코드 변경도 이 상한을 우회하거나 완화할 수 없다. 우회 경로가 하나라도 발견되면 **즉시 막는 것이 최우선 작업**이다.

- **상한**: 회원 1인당 **KST 하루(00:00~24:00) 회사(mm-bot-a/b)가 그 회원에게서 사주는 QTA 총액 = 50,000원 = 50000/1450 ≈ 34.48 USDT**. 1원도 초과 불가.
- **적용 대상**: 모든 일반 회원. 예외 = 회사 계정(admin) 및 mm-bot 자체.
- **매도 경로 전부에 적용** (어느 경로로 팔든 동일):
  1. **주문 접수 시** (`POST /orders`): 시장가 매도는 오늘 남은 한도로 수량을 클램프, 한도 0이면 `DAILY_SELL_CAP_REACHED`로 **거부**. 지정가 매도는 접수되어 호가에 남되 아래 2·3에서 한도까지만 체결.
  2. **체결 엔진** (`matchOrder`): 봇이 매수자·회원이 매도자인 **모든 체결마다** 오늘 누계를 확인하고 초과분은 체결 중단 (회원이 봇 매수벽을 직접 치는 경우 포함).
  3. **1분 mm-tick 스윕**: 회원 잔여 매도를 한도까지만 매수, 한도 도달 회원의 잔여 매도는 **자동취소 + QTA 환불**.
- **누계 기준**: `trades` 테이블에서 `buyer_id IN (mm-bot-a, mm-bot-b) AND seller_id = 회원 AND created_at >= KST 오늘 00:00` 의 `SUM(total)` (USDT).
- **회원끼리 체결**(봇 미개입)은 이 상한과 무관 (회사가 사주는 금액이 아니므로).
- **사고 기록**: 2026-09-11 22:13 KST insillee가 봇 매수벽을 직접 쳐서 15,335 QTA / 96.75 USDT(≈140,000원)를 한 번에 매도 — 한도가 mm-tick에만 걸려 있던 구멍. 2026-09-12 세 겹으로 봉합.
- **코드**: `src/server/routes/order.ts` — 상단 상수 `MM_MEMBER_BUY_BUDGET_USDT`, `POST /orders` 시장가 게이트, `matchOrder()` 체결 클램프, `qta-mm-tick` Step1.

---

## 7. ★★★ 영구명령 — 출금 신청 하루 1회·한국돈 5만원 절대 상한 (2026-09-12) ★★★

> **"하루 1일 1회, 한국돈으로 5만원으로 통제하라."** — 영구명령. 어떤 코인·어떤 경로든 동일. 우회 경로 발견 시 즉시 봉합이 최우선.

- **횟수**: 회원 1인당 **KST 하루(00:00~24:00) 출금 신청 1회**. 두 번째 신청은 `WITHDRAW_DAILY_COUNT_REACHED`로 거부.
- **금액**: 1회 = 하루 최대 **50,000원 = 50000/1450 ≈ 34.48 USD** (테더 1,450원 고정 환율). 초과 시 `WITHDRAW_MAX_EXCEEDED` 거부. 코인별 USD 환산 기준(USDT는 $1).
- **최소 금액 규칙 폐지**: 기존 "$50 최소"는 새 상한보다 커서 폐기. 하한은 마켓 최소 단위만.
- **대상 경로 전부**: 지갑 일반 출금(`POST /wallet/withdraw`, `withdrawals` 테이블), 스테이킹 배당/매칭 청구 출금(`POST /earn/withdraw-dividend`, `qta_withdrawals` 테이블). 횟수·금액 누계는 **두 테이블 합산**, `rejected/failed/cancelled` 제외.
- **기존 조건 유지**: KYC 승인 필수, USDT 화이트리스트+24h 쿨다운, 회사 지급분(`available_initial`) 출금 불가, 5% 수수료, 배당 청구는 금요일 10~16시 KST 창구.
- **코드**: `src/server/routes/wallet.ts` 상단 `WITHDRAW_MAX_KRW_PER_DAY / WITHDRAW_MAX_REQUESTS_PER_DAY`, `src/server/routes/earn.ts` withdraw-dividend, UI `WithdrawModal.tsx` / `EarnPage.tsx`.

---

## 8. 회원 매도 QTA 회수 → 회사 트레저리 → 메인 지갑 (2026-09-12 지상명령)

> "회원한테서 사준 QTA는 무조건 우리 메인 지갑으로 들어와야 한다."

- **A. 장부 회수(매일)**: mm-bot-a/b가 그날(KST) 회원에게서 사준 QTA 수량 전부를 봇 지갑 → **회사 트레저리 계정(admin)** 으로 내부 이체. `qta_treasury_sweeps`에 날짜·봇·수량·지급 USDT 기록. 봇끼리 코스메틱 체결은 제외(회원 매도만).
- **B. 온체인 이전(매일)**: 전날까지의 회수분을 **거래소 핫월렛(HD index 0) → 메인 지갑(`QTA_MAIN_PAYOUT_WALLET`)** 으로 네이티브 QTA 전송, tx_hash 기록. 조건: `QTA_CHAIN_DRIVER=real`, 니모닉이 핫월렛과 일치, 핫월렛 잔고 충분. 조건 미충족 시 장부 상태로 대기(유실 없음, 다음 날 재시도). 핫월렛 == 메인 지갑이면 전송 불필요로 기록.
- **실행**: 매일 03:00 UTC(12:00 KST) cron + 수동 `GET quantaex-cron…/treasury/sweep`. 현황 `GET …/treasury/report`.
- **현 상태(2026-09-12)**: 메인 지갑 설정값 = 핫월렛 주소(`0xdeB6…527b`) 동일 → B는 장부만 기록. 핫월렛 온체인 잔고 50,000 QTA. 별도 메인 지갑 주소를 지정하면 그 주소로 전송 시작.

---

## 9. 수당 계산 고정 단가 — 6원 → 10원 (2026-09-14 KST부터, 당분간)

> "내일부터는 당분간 수당은 6원에서 10원으로 계산해서 나가게 해줘라."

- **고정 단가 일정** (`src/shared/qta-peg.ts`, cron 사본 `cron-worker/src/qta-peg.ts`): 09-01~09-11 = **6원**, 09-12~13 = 공백(6원 유지 처리), **09-14 00:00 KST ~ 별도 지시까지 = 10원**. 테더 1,450원 고정.
- **적용 대상**: ① 데일리 수당(관리자 인정 포지션: `일 수당 USD ÷ 그날의 고정단가` → **날짜별 개별 계산**, 이미 지급된 날은 재계산 없음) ② 매칭보너스 USD→QTA 환산 ③ 배당/매칭 청구 출금 환산 ④ 지갑 출금 QTA 환산.
- **효과**: 같은 USD 수당이 QTA 수량으로는 **6/10 = 60%** 로 줄어듦 (예: $1,000 × 0.2% = $2/일 → 6원 시 483 QTA → 10원 시 290 QTA). 회원 화면에는 "fixed KRW 10 / 1,450" 표시(영문).
- **예외**: 실제 QTA를 잠근 포지션(`principal_qta>0` 또는 `qta_price_at_stake>0`)은 수량 고정 규칙 유지(1건).
- **전수조사**: `GET quantaex-cron…/peg-census` — 활성 포지션별 6원/10원 기준 일 수당 비교.

---

## 10. 테더(USDT) 입금 자동승인 + 입금자 식별 (2026-09-14 지상명령)

> "테더 입금은 자동승인제로 변경한다. 누가 입금했는지 확실히 알 수 있도록 반드시 조치하고!"

- **자동승인**: 온체인 USDT(BEP-20) 입금이 필요 컨펌(BSC 15)에 도달하면 **관리자 승인 없이 즉시 회원 지갑에 반영**. 2026-08-29의 "관리자 승인 필수" 규칙은 폐지. (`ext_deposits.approved_by = 'auto'`)
- **입금자 식별 — 구조적으로 확정**: 회원마다 **전용 입금주소**(HD 파생, `ext_addresses.user_id`)가 있으므로 "어느 주소로 들어왔나 = 누가 입금했나". 회원 주장이 아닌 주소 소유로 판별.
- **모든 자동반영 건에 4중 기록**: ① `ext_deposits`(회원ID·입금주소·보낸지갑 from·Tx·블록·금액) ② `deposits` 장부(`ext:bep20:<tx>`) → 회원 지갑 내역/wallet-debug 표시 ③ 회원 앱 알림 "Deposit Credited" ④ **Audit 탭** `ext_deposit.auto_credit` (admin_id `system:ext-watcher`, payload에 닉네임·이메일·금액·From·To·Tx·블록).
- **관리자 화면**: Deposits 탭 → On-chain: 기본 '입금 완료(자동)' 목록, 열 = 회원(닉네임·이메일·ID) / 금액 / 네트워크 / **From → 입금주소** / Tx / 컨펌 / 상태(자동 반영). '승인 대기(구)' 탭은 이전 규칙 잔여 건 수동 처리용.
- **★ 입금 소리 알림 (2026-09-14)**: 관리자 화면 상단 헤더 **Deposit bell** — 자동반영된 입금이 생기면 10초 내 **띵동 차임 + 음성 "띵동, 테더가 입금되었습니다 (닉네임, 금액)"** + 토스트(회원·금액·From) + 탭이 뒤에 있으면 브라우저 알림. 브라우저 정책상 최초 1회 벨을 클릭해 소리를 켜야 하며(빨간 점멸 → 초록 ON), 이후 유지. 관리자 콘솔 전용이므로 음성은 한국어.
- **★ 모바일 입금 모니터 (2026-09-14)**: **`https://www.quantaex.io/admin/deposits`** — 관리자 로그인 후 테더 입금만 보이는 한 화면(소리 켜기 타일 / 오늘 건수·USDT 합계 / 최근 자동반영 목록: 닉네임·이메일·금액·From·Tx·시각). 10초 폴링, 새 입금마다 띵동+음성+진동+토스트+브라우저 알림. 소리 켜면 화면 꺼짐 방지(Wake Lock). 홈화면에 추가해 쓰면 됨. 로그인 페이지는 `?next=`로 되돌아옴.
- **코드**: `cron-worker/src/ext-watcher.ts` extDepositTick, `src/server/routes/admin.ts` GET /ext-deposits(from_address) · GET /ext-deposits/recent, `pages/AdminDepositsMobilePage.tsx`, `AdminPage.tsx` Deposits On-chain, `components/admin/DepositBell.tsx`.

---

## 11. QTA 입금 허용 — 거래소 지분자 · 카지노 지분자 한정 (2026-09-21 지상명령)

> "QTA 입금을 받아주는 기능을 만들고! 거래소 지분자, 카지노 지분자는 보유한 qta 입금시 하루 5만원 매도가 가능하게 만들어줘! 거래소 지분자와 카지노 지분자는 어드민 관리자가 결정하게 해주고!"

- **★ 2026-09-21 추가 지시 — "일단 다 받고 보자. 단, 언제든 돌려줄 수 있게 누가 언제 몇개를 보냈는지 관리자에 다 떠야 한다."**
  - **모든 회원**이 전용 입금주소로 보낸 네이티브 QTA는 **전부 온체인 수령·기록**하고 회사 메인지갑으로 스윕한다(코인은 받는다).
  - **지분자** → 컨펌 시 자동으로 QTA 지갑 반영. **일반 회원** → 장부 상태 **`held`(회사 보관, 미반영)** — 잔액에 넣지 않고 회사가 보관. 회원 앱에는 "QTA Received (on hold)" 알림 + 입금 내역 "On hold (exchange)".
  - **관리자 화면**: Admin → Deposits → **「QTA 입금 보관」탭** — 모든 QTA 입금을 회원(닉네임·실명·이메일·지분자 여부) / **보낸 지갑(From)** / **온체인 시각(KST)** / **수량** / 입금 Tx·블록 / 컨펌 / 상태 / 메인지갑 스윕 Tx / 처리 기록으로 표시. 상단 합계(보관·반영·컨펌중·반환). 검색(이메일·닉네임·실명·Tx·지갑).
  - **처리 버튼**(보관 건만): **반영** → 회원 QTA 지갑에 넣음(이후 6번 5만원 매도 상한 적용, Audit `qta_deposit.admin_credit`) / **반환 기록** → 메인지갑에서 보낸 지갑으로 전송 후 **Tx 해시 필수 입력**으로 기록(Audit `qta_deposit.returned`, 회원 알림 "QTA Returned"). 둘 다 상태 가드로 중복 불가.
  - 저장 컬럼(마이그레이션 0058, cron 자동 적용): `from_address, chain_ts, held_at, sweep_tx_hash, swept_at, resolution, resolved_at, resolved_by, resolution_note, return_tx_hash`.
- **★ 2026-09-21 추가 지시 — "버튼 한 번으로 메인지갑에서 자동 반환 송금까지. 아닌 사람은 바로 반환."**
  - **자동 반환 스위치**(`system_state.qta_auto_return`, 기본 **ON**): ON이면 비지분자의 QTA는 컨펌 도달 즉시 `return_pending` → 크론이 **회사 핫월렛(HD #0)에서 보낸 지갑(from_address)으로 동일 수량 자동 송금** → `returned` (return_tx_hash·return_to 기록, 회원 알림 "QTA Returned + Tx", Audit `qta_deposit.auto_returned`). OFF면 `held` 보관 후 관리자 결정.
  - **버튼 한 번 반환**: 「QTA 입금 보관」탭 보관 건의 **⚡ 자동 반환** 버튼 → 즉시 대기열 등록 + 크론 워커 `/qta/return` 즉시 호출(수 초 내 Tx). 「수동 기록」(직접 송금 후 Tx 입력)과 「반영」도 유지. `return_pending` 건은 송금 전이면 「반환 취소 → 반영」 가능.
  - **안전장치**: 틱당 1건 송금, 상태 가드로 중복 송금 불가, 보낸 주소가 우리 핫월렛/회원 입금주소면 반환 금지(루프 방지), 실패 3회 → `held`로 복귀 + `return_error` 표시, 핫월렛 가스 0.1 QTA 예비 유지. 니모닉↔핫월렛 불일치 시 송금 안 함.
  - 코드: `cron-worker/src/qta-return.ts`, `/qta/return` 엔드포인트, `/admin/qta-deposits/:id/auto-return`, `PUT /admin/qta-deposits/auto-return`, 마이그레이션 0059.
- **원칙 유지**: 일반 회원은 여전히 **QTA 입금이 잔액으로 반영되지 않음(출금 전용, 2026-08-28 규칙)**. USDT를 입금해 시장에서 매수하는 것만 가능. 보낸 QTA는 위와 같이 회사 보관 상태로 기록되며 관리자가 반영/반환을 결정.
- **예외 — 지분자**: 관리자가 **거래소 지분자** 또는 **카지노 지분자**로 지정한 회원은 **본인 보유 QTA(네이티브, Quantarium 체인)를 전용 입금주소로 입금**할 수 있고, 필요 컨펌(메인넷 12) 도달 시 **자동으로 QTA 지갑에 반영**된다.
- **매도 한도**: 입금된 QTA의 매도는 **6번 영구명령(회원당 하루 5만원 = 34.48 USDT 회사 매입 상한)** 그대로 적용. 별도 한도 없음 — 지분자도 하루 5만원까지만 회사가 사준다. 출금은 7번 규칙(하루 1회·5만원) 동일.
- **결정권**: 지분자 여부는 **어드민 관리자가 결정**. Admin → Users 탭 **"지분자" 열**의 `거래소` / `카지노` 토글, 또는 회원 상세 모달의 **"지분자 지정" 패널**(ON/OFF). 변경 시 Audit(`user.set_shareholder`) 기록 + 회원 앱 알림.
- **플래그 저장**: `users.fee_exempt_exchange_holder`, `users.fee_exempt_casino_holder` (마이그레이션 0051, cron 자동 적용에 추가). 이 플래그는 **수수료와 무관**(수수료는 QX+QKEY 보유량 기준 유지) — 오직 QTA 입금 허용 여부만 결정.
- **회원 화면(영문)**: 지분자는 Deposit 모달 코인 목록에 QTA가 나타나고 Quantarium 네트워크 입금주소 + 안내("Deposited QTA can be sold up to KRW 50,000 per day")가 표시. 비지분자는 기존 "QTA is withdraw-only" 안내 유지. 입금 내역(Deposit History)에 Quantarium 체인 입금(QTA/QX/QKEY) 표시.
- **트레저리 회수**: 지분자가 입금한 네이티브 QTA도 8번 규칙에 따라 스윕 대상 — 토큰(QX/QKEY) 스윕이 끝난 주소의 네이티브 QTA 잔액(가스 제외, 0.01 QTA 이상)을 회사 메인 지갑으로 전송.
- **식별 기록**: `qta_deposits.raw_meta`에 from 주소·user_id·`accepted_reason: 'shareholder'`·`rule: 'OWNER_RULES §11'` 저장(10번의 입금자 식별 원칙 준수).
- **코드**: `src/shared/shareholder.ts`(+ `cron-worker/src/shareholder.ts` 복사본), `cron-worker/src/index.ts` scanQtaDeposits(지분자만 `listInboundNativeTxs`) · qtaChainTick(알림) · sweepQtaDeposits(네이티브 스윕), `cron-worker/src/migrate.ts` 0051, `src/server/routes/admin.ts` POST /users/:id/shareholder · GET /users(+플래그), `src/server/routes/auth.ts` /me(`can_deposit_qta`), `src/server/routes/chain.ts` /qta/deposit-address(QTA 지분자 허용), `src/server/routes/wallet.ts` /history/deposits(qta_deposits 병합), `src/components/wallet/DepositModal.tsx`, `src/utils/networks.ts` getDepositNetworks(allowQta), `src/pages/AdminPage.tsx` UsersTab · ShareholderPanel.

---

## 12. ★★★ 영구명령 — QTA 매도는 사전 승인 회원만 · 매도 현황 표시 · 출금 신청 금요일 창구 (2026-09-21) ★★★

> "사전 매도가 승인된 회원만 매도가 가능하다고 이전에 분명히 지시했다. 잊으면 안 된다!"
> "매일 5만원어치 시세에 따라 매도가 가능해야 한다. 그걸 바로 USDT로 얼마인지 누적과 함께 표시해 주고! 본인 지갑으로 출금 신청은 금요일 해당 시간에만."
> "지분자(거래소/카지노)로 등록된 사람은 자동으로 매도 승인된 걸로 포함시켜라."

- **매도 사전 승인**: `users.qta_sell_approved = 1`(관리자 지정) **또는** 지분자(`fee_exempt_exchange_holder` / `fee_exempt_casino_holder` = 1)인 회원만 QTA 매도 가능. 그 외 전원 **매도 불가** (기본값). 회사 계정·봇 제외.
  - **3겹 강제**: ① 주문 접수(`POST /orders`) → `SELL_NOT_APPROVED` 403 (시장가·지정가·스탑 전부) ② 체결 엔진(`matchOrder`) → 미승인 회원 매도는 테이커든 메이커든 체결 스킵 ③ mm-tick 1분 스윕 → 미승인 회원의 잔여 매도 주문 전량 자동 취소·환불. 승인 해제 시 즉시 미체결 매도 주문 취소·환불.
  - **관리자**: Admin → Users 「매도승인」열(승인/불가/자동) 토글 + 회원 상세 「QTA 매도 승인」패널. `GET /admin/users/sellers` = 현재 매도 가능 회원 명단(보유·오늘 매도·누적). Audit `user.sell_approve` / `user.sell_revoke`, 회원 알림.
  - 지분자 지정 = 매도 자동 승인 (별도 토글 불필요, 화면에 "자동" 표시).
- **매도 한도·표시**: §6 그대로 — 승인 회원은 **시세로 하루 5만원(=34.48 USDT, 1,450원 고정)**까지 회사가 매입. 거래 화면 매도 탭에 **일일 한도 / 오늘 매도(USDT·QTA) / 남은 한도(USDT ≈ QTA) / 누적 매도(전체 USDT·QTA·건수)** 실시간 위젯(15초 갱신, `GET /orders/qta-sell-status`). 미승인 회원은 매도 탭에 "사전 승인 필요" 안내 + 매도 버튼 비활성.
- **출금 신청 창구**: 본인 지갑으로의 출금 신청(`POST /wallet/withdraw`, 모든 코인)은 **매주 금요일 10:00~16:00 KST에만** 접수 (배당 청구 §7 창구와 동일). 그 외 → `WITHDRAW_WINDOW_CLOSED` 403 + 다음 창구 시각 안내. §7 상한(하루 1회·5만원) 병행. 회사 계정 제외. 출금 모달 상단에 창구 열림/닫힘·다음 시각 표시. `GET /wallet/withdraw-window`.
- **코드**: `src/shared/shareholder.ts`(canSellSql/loadSellApproval, cron 복사본 동일), `src/server/routes/order.ts`(POST 게이트·matchOrder·mm-tick·/qta-sell-status), `src/server/routes/wallet.ts`(withdrawWindowOpen·/withdraw-window), `src/server/routes/admin.ts`(/users/:id/sell-approval·/users/sellers), `src/components/trade/TradePanel.tsx`, `src/components/wallet/WithdrawModal.tsx`, `src/pages/AdminPage.tsx`, 마이그레이션 0060.

---

## 13. KYC 이중 인증 — 이메일 6자리 + SMS 6자리 (2026-09-22 지시)

> "KYC에서 텔레그램 인증하듯 해외 인증을 날리고 싶다. 인증번호 6개짜리. 이메일 인증하고 SMS 인증하는 이중방식."

- **흐름**: KYC 1단계에서 ① 가입 이메일로 6자리 코드 → 입력 ② 국가번호(+81/+82/…) 선택 + 휴대폰 번호 → SMS 6자리 코드 → 입력. **둘 다 인증돼야** 다음 단계·제출 가능(서버 게이트 `VERIFICATION_REQUIRED`). 번호를 바꾸면 SMS 재인증.
- **비용 통제 (오너: SMS는 돈이다)**: 회원이 버튼을 눌러 요청할 때만 발송(일괄 발송 없음), 채널별 60초 재전송 쿨다운, 하루 5회 한도, 코드 10분 유효, 5회 오입력 시 폐기. 실패 발송은 쿨다운에 안 걸림. 예상: KYC 제출자 수 × 약 $0.08.
- **발송사**: 이메일 = 기존 Resend(무료). SMS = **Twilio** — Cloudflare Pages 환경변수 `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_FROM`(발신번호 또는 Sender ID "QuantaEX") 또는 `TWILIO_MESSAGING_SERVICE_SID`. **미설정 시 개발모드**: 코드가 응답·서버 로그에 표시돼 흐름 테스트 가능(회원 화면에 "Test mode" 표시). `KYC_SMS_DEV_MODE=true`로 강제 개발모드.
- **★ 관리자 SMS 설정 패널 (Admin → KYC 탭 상단)**: Cloudflare 환경변수 대신 **Account SID + Auth Token을 붙여넣으면 서버가 자동 처리** — ① 자격증명 검증(Twilio 계정 조회) ② 발신번호 없으면 미국 SMS 번호 자동 구매(월 ~$1.15) ③ 테스트 발송 ④ LIVE 전환. 잔액·보유 번호·최근 발송 결과 표시, 실발송 ON/OFF 스위치. 토큰은 `system_state.twilio_config`에만 저장되고 화면에 재표시되지 않음. 환경변수(`TWILIO_*`)는 폴백. Audit `sms.*`.
- **Twilio 콘솔에서 오너가 할 것**: Geo permissions(Messaging → Settings)에서 Japan·South Korea 등 회원 국가 허용. Auto-recharge는 OFF 유지.
- **★ 2026-09-22 발송사 전환 — Vonage 권장**: Twilio는 한국 계정에 KCB 본인인증(Trust Hub)을 강제하는데 그 iframe이 Chrome·Edge 모두에서 차단돼 진행 불가(에러 20003, 번호 구매·발송 전면 차단). 관리자 SMS 패널에 **Vonage(Nexmo) 지원 추가** — API key/secret 붙여넣기 → 잔액 조회로 검증 → 즉시 LIVE. 발신자명 `QuantaEX`(번호 구매 불필요, 사전 심사 없음). 저장 형식 `system_state.twilio_config = {provider:'vonage'|'twilio', sid, token, from, enabled}`. Twilio 계정($50)은 Trust Hub 풀리면 패널에서 전환 가능. Twilio 토큰이 채팅에 노출됨 → **오너가 Regenerate 필요**.
- **관리자**: KYC 대기 카드에 ✉ 이메일 인증/미인증 · 📱 SMS 인증(+E.164)/미인증 · 신분증/주소증명 첨부 여부 뱃지. §13 이전 제출 42건은 둘 다 "미인증"으로 표시됨.
- **저장**: `kyc_verifications`(코드 해시·발송사·오류·IP), `users.kyc_email_verified_at / kyc_phone_verified_at / kyc_phone_e164`. 마이그레이션 0061.
- **코드**: `src/server/lib/kyc-verify.ts`, `src/server/routes/profile.ts`(/kyc/verify/status·send·confirm + 제출 게이트), `src/pages/KycPage.tsx`, `src/server/routes/admin.ts` /kyc/pending.

---

## 변경 이력
- 2026-08-31: 최초 작성. 관리자 인정 스테이킹(총금액 데일리·매칭 / 실입금 반환), 데일리 KST 자정 기준, 매칭 5단계 1회성 규칙 못박음.
- 2026-09-04: **바이너리 규칙 근본 정정.** ① 좌우 볼륨은 무한대(하부 실적 전부 반영, 이전 볼륨 2× 하드캡·드롭 폐기). ② 매칭수당 총 한도 = 본인 몸값 × 2(USD 누계, 좌우 통합). ③ 데일리와 매칭 200% 한도는 별개. 전체 라인 볼륨 재산정(namim 좌 $2,000→$6,000 등), 매칭 지급 총액 불변(268,250 QTA).
- 2026-09-04: **지상명령 문서화(코드 이미 준수, 문서만 보강).** ① 데일리 배당은 **포지션별 개별 계산**(자기 요율·기간·시작일, 합산 금지) — 2번 규칙에 명시. ② 몸값(self_usd)은 **진입 시마다 누적 합산**(예: $1,000 + 한 달 뒤 $5,000 = 그날부터 $6,000) → 하부 좌/우 각각 **$12,000까지 진입** 가능(매칭 한도 기준도 $12,000) — 3번 규칙에 명시. 코드 검증: `accruedQta`/`accruedDays`(포지션별), `self_usd = self_usd + ?`(binary-matching.ts:317, 누적), `matchCapUsd = selfUsd × 2` — 전부 준수 확인.
- 2026-09-05: **QTA 거래/시장운영 정책 신설(4번 섹션).** 테더 1,450원 고정 / 24h 평균 하한선 이하 매도 체결 금지(어뷰징 방어) / 매수 무제한(순수매도인 먼저→회사 매수인가격 공급, 시세·하한선 미만은 공급안함) / 매도는 회사가 회원당 하루 5만원어치까지 실시간 매수 / 체결은 1분 주기 cron으로 1~59초 내. matchOrder 하한선 게이트 + qta-mm-tick 재작성 + cron 1분 트리거 추가. 매도 일일한도 환율 1400→1450, POST 단계 거부 폐지.
- 2026-09-05: **매도 초과분 정책 수정 — 방치 → 자동취소·환불.** 하루 회사매수 한도(5만원) 도달 회원의 잔여 QTA 매도주문을 mm-tick에서 전량 자동 취소하고 잠긴 QTA를 available로 환불(호가창에서 제거). `qta-mm-tick` Step1에 auto-cancel sweep 추가.
- 2026-09-10: **QTA 일일 시세 플랜(5번 섹션) 신설.** "오늘은 0.006 중심 진동, 마감 0.0058" 지시를 mm-bot 1분 틱이 자동 실행(ramp→oscillate→close→hold→carry). 관리자 API/UI 추가, 헤더 시세 클램프를 플랜 엔벨로프로, 24h 하한선은 봇↔봇 코스메틱 체결 예외. 기본 플랜 코드 내장(배포 즉시 적용).
- 2026-09-11: **일일 플랜 갱신** — 중심 0.007 ±2.5%, 23:00→23:55 마감 0.0065. 코드 기본 플랜(DEFAULT_PLAN)은 날짜가 더 새로우면 저장된 플랜/Clear를 자동 대체(최신 지시 우선).
- 2026-09-12: **★ 영구명령 6번 신설 — QTA 회원 매도 하루 5만원 절대 상한.** 주문 접수/체결 엔진/mm-tick 3겹 강제. insillee 14만원 우회 체결 사고 후 봉합.
- 2026-09-12: **★ 영구명령 7번 신설 — 출금 신청 하루 1회·5만원 절대 상한.** 지갑 출금·배당 청구 출금 모두 적용, 두 테이블 합산 카운트. 기존 $50 최소 규칙 폐지 (상한보다 커서).
- 2026-09-12: **8번 신설 — 회원 매도 QTA 회수 루틴.** 봇→트레저리 장부 이체(A) + 핫월렛→메인지갑 온체인(B) 일일 스윕, treasury/sweep·report 엔드포인트.
- 2026-09-13: **9번 신설 — 수당 고정단가 6원→10원(09-14~ 당분간).** 공용 peg 스케줄 모듈 도입, 데일리 수당은 날짜별 단가로 개별 합산, 매칭/청구/지갑 환산 일괄 적용. peg-census 엔드포인트.
- 2026-09-14: **Day Plan 관리자 OHLC 컨트롤** — Open/Centre/High/Low/Close 달러 입력, High/Low 하드 클램프, 등락폭 자동 산출. 월요일 기본 플랜에 H 0.0093 / L 0.0084 명시.
- 2026-09-14: **Day Plan 다중 날짜 스케줄** — 날짜별 템플릿 저장, 00:00 KST 자동 승격, 관리자 주간 표 UI + API(GET/PUT/DELETE /admin/coins/QTA/day-plan/schedule).
- 2026-09-14: **10번 신설 — USDT 입금 자동승인 + 입금자 4중 식별 기록.** 관리자 승인 게이트(08-29) 폐지.
- 2026-09-14: **관리자 입금 벨** — 자동반영 입금 시 띵동 + 한국어 음성 + 토스트 + 브라우저 알림 (헤더 Deposit bell, 최초 1회 클릭 활성).
- 2026-09-14: **모바일 입금 모니터 /admin/deposits** — 테더 입금 전용 단일 화면, 띵동/음성/진동/알림, Wake Lock.
- 2026-09-17: **4일 마감 지시 코드 내장(DEFAULT_SCHEDULE)** — 목 0.0091 / 금 0.0086 / (토 미지정→0.0086 유지) / 일 0.0092 / 월 0.0102, 모두 23:59 도달. 각 날 램프→진동(덤프/회복)→마감. 어드민 스케줄이 있으면 그것이 우선.
- 2026-09-21: **11번 신설 — QTA 입금 허용(지분자 한정).** 거래소/카지노 지분자(관리자 지정)만 본인 QTA 네이티브 입금 자동반영, 매도는 6번 5만원 상한 동일 적용. Admin Users 지분자 토글 + 회원 상세 패널, /me can_deposit_qta, Deposit 모달 QTA 노출, 네이티브 QTA 트레저리 스윕, 0051 자동 마이그레이션.
- 2026-09-21: **11번 보강 — QTA 입금 전부 수령·보관 장부.** 모든 회원 QTA 입금 온체인 수령+메인지갑 스윕, 지분자 자동반영/일반회원 `held` 보관, Admin Deposits「QTA 입금 보관」탭(누가·언제·몇개·From·Tx·스윕·처리), 반영/반환기록 버튼(Audit), 0058 마이그레이션.
- 2026-09-21: **11번 보강 — 비지분자 QTA 자동 반환.** 스위치 ON(기본) 시 컨펌 즉시 핫월렛→보낸 지갑 자동 송금(Tx 기록·알림·Audit), 보관 건 ⚡ 자동 반환 버튼(즉시 실행), 실패 재시도 3회, 0059 마이그레이션.
- 2026-09-21: **★ 12번 신설 — QTA 매도 사전 승인 회원만(지분자 자동 포함) 3겹 강제 + 매도 현황 위젯(오늘/남은/누적 USDT) + 본인 지갑 출금 신청 금요일 10~16시 KST 창구.** 이전 지시가 코드에 누락돼 있던 것을 점검 후 봉합. 0060 마이그레이션.
- 2026-09-22: **13번 신설 — KYC 이메일+SMS 6자리 이중 인증.** 요청 시에만 발송·쿨다운·일일 한도로 비용 통제, Twilio 미설정 시 개발모드, 관리자 KYC 카드에 인증 뱃지. 0061 마이그레이션.
- 2026-09-22: **13번 보강 — 관리자 SMS(Twilio) 설정 패널.** SID/토큰 붙여넣기 → 검증·번호 자동구매·테스트·LIVE 전환 원클릭. 잔액/번호/발송 이력 표시.
- 2026-09-22: **13번 보강 — SMS 발송사 Vonage 지원.** Twilio KCB Trust Hub 차단으로 전환. 관리자 패널 provider 선택(Vonage 권장), 잔액 검증, 발신자명 QuantaEX.
